...
@Config
@TeleOp(name = "TeleOp-1 pad")public class Teleop_test_kla_3 extends LinearOpMode {
    ...
    private boolean IntakeR_Pressed = false;
    @Override
    public void runOpMode() {
        ...
        if (opModeIsActive()) {
            while (opModeIsActive()) {
                ...
                if(!gamepad1.b && Outtake_Pressed){
                    Outtake_Pressed = false;
                    OuttakeDc.setPower(0);
                }
                if(gamepad1.x){
                    IntakeDc.setPower(-intakePower);
                    IntakeR_Pressed = true;
                }
                if(!gamepad1.x && IntakeR_Pressed){
                    IntakeDc.setPower(0);
                    IntakeR_Pressed = false;
                }
                telemetry.update();
            }
        }
    ...
    void initialize() {
        ...
        OuttakeDc.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
